        ...
        try:
            ...
            async def step_callback(text, screenshot_filename):
                await websocket.send(json.dumps({
                    "sender": "agent",
                    "text": text,
                    "screenshot": encode_screenshot(screenshot_filename) if screenshot_filename else None
                }))

            login_status = await controller.login(conversation.context['email'], conversation.context['password'], step_callback=step_callback)
            await websocket.send(json.dumps({
                "sender": "agent",
                "text": login_status,
                "screenshot": encode_screenshot("inbox_loaded.png")
            }))
            if "successful" not in login_status:
                await websocket.send(json.dumps({
                    "sender": "agent",
                    "text": "Login failed. Please check your credentials or for additional verification.",
                    "screenshot": encode_screenshot("gmail_login_error.png")
                }))
                break
            ...
            send_status = await controller.compose_email(
                conversation.context['recipient'], subject, body, step_callback=step_callback)
            await websocket.send(json.dumps({
                "sender": "agent",
                "text": send_status,
                "screenshot": encode_screenshot("sent_confirmation.png")
            }))
